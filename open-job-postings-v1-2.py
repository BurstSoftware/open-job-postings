...
with st.sidebar:
    ...
    page = st.selectbox(
        "Navigate",
        ["🏠 Home", "🔍 Discover Jobs", "📝 Post a Job", "📬 My Applications", "👔 Employer Hub", "🤖 AI Matcher"],
...
if page == "🏠 Home":
    ...
    st.image("https://picsum.photos/id/1015/1200/400", use_column_width=True)  # Placeholder hero
    ...
    with col1:
        search = st.text_input("🔎 Search titles, skills, companies...", placeholder="Senior Engineer, React, Remote")
    with col2:
        location = st.selectbox("📍 Location", ["All Locations", "Remote", "New York", "San Francisco", "London"])
    with col3:
        job_type = st.selectbox("💼 Type", ["All Types", "Full-time", "Contract", "Part-time"])
    with col4:
    ...
    df = df[df['salary'].str.extract(r'(\d+)').astype(float)[0] >= min_salary]
            ...
            with st.container():
                ...
                <div class="job-card">
                    ...
                    <div style="display:flex; gap:12px; margin-top:20px;">
                        <button onclick="window.alert('Applied! 🎉')" 
                                style="background:#4a5dff; color:white; border:none; padding:10px 28px; border-radius:50px; font-weight:600; cursor:pointer;">
                            Apply Now
                        </button>
                    </div>
                </div>
                ...
                # Real button (hidden behind card for interaction)
    ...
    resume = st.text_area("Your resume / skills summary", height=220, 
